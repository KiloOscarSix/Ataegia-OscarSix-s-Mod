...
screen input(prompt, someText = "What's your name?"):
...
screen main_menu():
    ...
    #$ renpy.movie_cutscene("mm.webm")
    add Movie(size=(1920, 1080))
    on "show" action Play("movie", "gui/mm.webm", loop=True)
    on "hide" action Stop("movie")
    on "replace" action Play("movie", "gui/mm.webm", loop=True)
...
screen history():
    ...
    use game_menu(_("Player Stats"), scroll=("vpgrid" if gui.history_height else "viewport"), yinitial=1.0):
...
label get_save_name:
    ...
    if not renpy.variant("pc"):
        $ save_name_tmp = renpy.call_screen("modal_input", prompt= _("{size=-20} Name your savegame: {/size}"), default = "", length=35)
        $ save_name = save_name_tmp.content if isinstance(save_name_tmp, Text) else save_name_tmp
    else:
        $ save_name = renpy.call_screen("modal_input", prompt= _("{size=-20} Name your savegame: {/size}"), default = "", length=35)

    return

init -4 screen modal_input:
    modal True
    window style "input_window":
        ...
        if not renpy.variant("pc"):
            hbox:
                ...
                vbox:
                    xalign 0.0
                    input id "input" style "input_text" length 35
                vbox:
                    xalign 1.0
